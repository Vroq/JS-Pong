...
var ctx = canvas.getContext('2d'); // The HTMLCanvasElement.getContext() method returns a drawing context on the canvas. "2d", leading to the creation of a CanvasRenderingContext2D object representing a two-dimensional rendering context.
var pong = {};

// konstruktor obiektu pilka
function Pilka(promien, kolor) {
   this.x       = 0, // Variables with this become public variables while those with var become private variables. The variables or function declared with this keyword become instance members of the class which means they will be available in each newly created instance of that class
   this.y       = 0,
   this.offsetX = 0,
   this.offsetY = 0,
   ...
   this.rysuj   = function() {
       ...
       ctx.arc(this.x, this.y, this.promien, 0, Math.PI*2); // arc(x, y, radius, startAngle, endAngle, counterclockwise)
       ctx.fillStyle = this.kolor; // property of the Canvas 2D API specifies the color, gradient, or pattern to use inside shapes. The default style is #000 (black).
...
function Rakietka(nazwa, szerokosc, dlugosc, offset, kolor) {
   ...
   this.rysuj     = function() {
       ctx.beginPath();
       ctx.rect(this.x, this.y, this.szerokosc, this.dlugosc); // rect(x, y, width, height), The rect() method creates a rectangular path whose starting point is at (x, y) and whose size is specified by width and height.
...
function graInit() {
   ...
   pong.pauza     = true;
   pong.pilka     = new Pilka(10, '#fff');
   pong.gracze    = [];
   pong.gracze[0] = new Rakietka('Gracz Lewy',  15, 60, 7, '#fff');
   pong.gracze[1] = new Rakietka('Gracz Prawy', 15, 60, 7, '#fff');
   pong.liczbaZyc = 10;
   pong.zwyciezca = 0;
   graReset();
}
...
function keyDownHandler(e) {
   if (e.keyCode == 65) { pong.gracze[0].doGory = true; } else // a = 65
   if (e.keyCode == 75) { pong.gracze[1].doGory = true; } else // k = 75
   if (e.keyCode == 90) { pong.gracze[0].doDolu = true; } else // z = 90
   if (e.keyCode == 77) { pong.gracze[1].doDolu = true; } else // m = 77
...
// weryfikacja czy rakietka odbija pilke  https://goalkicker.com/HTML5CanvasBook/ Chapter 11: Collisions and Intersections
